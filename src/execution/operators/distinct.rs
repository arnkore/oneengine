...
/// 去重优化器
pub struct DistinctOptimizer {
    /// 统计信息收集器
    stats_collector: std::collections::HashMap<String, DistinctStats>,
}
...
impl DistinctOptimizer {
    ...
    /// 选择最佳去重算法
    pub fn choose_distinct_algorithm(
        &self,
        input_rows: u64,
        distinct_columns: usize,
        memory_limit: usize,
    ) -> DistinctAlgorithm {
        let estimated_memory = input_rows * distinct_columns as u64 * 50; // 估算每列50字节
        
        if estimated_memory < memory_limit as u64 {
            DistinctAlgorithm::HashSet
        } else {
            DistinctAlgorithm::SortDistinct
        }
    }
    
    /// 更新统计信息
    pub fn update_stats(&mut self, distinct_id: String, stats: DistinctStats) {
        self.stats_collector.insert(distinct_id, stats);
    }
}
...
mod tests {
    ...
    fn test_distinct_basic() {
        ...
        let batch = RecordBatch::try_new(
            ...
            vec![
                Arc::new(Int32Array::from(vec![1, 2, 1, 3, 2])),
                Arc::new(Int32Array::from(vec![10, 20, 10, 30, 20])),
            ],
        ).unwrap();
        
        let config = DistinctConfig::new(
            Arc::new(schema),
            vec!["id".to_string(), "value".to_string()],
        );
        
        let mut distinct = VectorizedDistinct::new(
            config,
            1,
            vec![0],
            vec![0],
            "test_distinct".to_string(),
        ).unwrap();
        
        let mut out = crate::execution::push_runtime::Outbox::new();
        let status = distinct.process_batch(batch, &mut out);
        assert!(status.is_ok());
    }
}
