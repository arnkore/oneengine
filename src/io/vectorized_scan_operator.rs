...
impl VectorizedScanOperator {
    ...
    /// 应用谓词下推
    pub fn apply_predicate_pushdown(&mut self, predicates: Vec<PredicateFilter>) -> Result<(), String> {
    ...
    pub fn apply_column_projection(&mut self, columns: Vec<String>) -> Result<(), String> {
        ...
        // 创建投影掩码
        let projection_mask = ProjectionMask::roots(metadata.file_metadata().schema_descr(), column_indices);
        
        // 创建Arrow读取器
        let arrow_reader = ParquetRecordBatchReader::try_new(
            reader,
            projection_mask,
        ...
        // 更新配置
        self.config.projection_columns = Some(columns);
        self.arrow_reader = Some(arrow_reader);
